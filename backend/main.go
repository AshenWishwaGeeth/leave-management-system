...
func main() {
    ...
    // Try the configured port first, then a few adjacent ports if already in use.
    for offset := 0; offset < 5; offset++ {
        ...
        if strings.Contains(errText, "address already in use") || strings.Contains(errText, "only one usage of each socket address") {
